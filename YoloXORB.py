...
cap = cv2.VideoCapture("PlaneDifficult.mp4")
        ...
        if not yolo_detected_objects:
            ...
            if des_last is not None and des_current is not None:
                ...
                for match in matches:
                    matched_kp = kp_current[match.trainIdx].pt  # Get the matched keypoint's position

                    # Draw a circle for the matched keypoint
                    cv2.circle(frame, (int(matched_kp[0]), int(matched_kp[1])), 5, (255, 0, 0), 2)
                    cv2.putText(frame, label, (int(matched_kp[0]), int(matched_kp[1]) - 10), cv2.FONT_HERSHEY_SIMPLEX,
                                0.9, (0, 0, 255), 2)
